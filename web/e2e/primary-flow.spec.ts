...
test('quick edit validates trim and submits combined rotate and mute with honest preview fallback', async ({ page }) => {
  ...
  await dialog.getByLabel('Trim', { exact: true }).check()
  ...
  await dialog.getByLabel('Rotate clockwise').selectOption('90')
...
for (const [locale, convert, edit, submit, rotate, crop, resize, volume, acknowledge] of [
  ...
  test(`export dialogs are localized, focus trapped and contained in ${locale}`, async ({ page }) => {
    ...
    await page.getByRole('menuitem', { name: edit, exact: true }).click()
    const dialog = page.getByRole('dialog', { name: edit, exact: true })
    await dialog.getByLabel(rotate, { exact: true }).selectOption('90')
    await dialog.getByLabel(crop, { exact: true }).check()
    await dialog.getByLabel(resize, { exact: true }).check()
