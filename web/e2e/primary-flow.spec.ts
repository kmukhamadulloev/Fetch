...
test('metadata menu opens modal, advanced fields and dirty guard without playback', async ({ page }) => {
  await mockApi(page)
  await mockMetadata(page)
  await page.goto('/completed')
  const pencil = page.getByRole('button', { name: 'Actions for Fixture media' })
  await pencil.click()
...
test('audio metadata saves changed tags and uploaded artwork with responsive footer', async ({ page }) => {
  ...
  await page.goto('/completed')
  await page.getByRole('button', { name: 'Actions for Fixture media' }).click()
  await page.getByRole('menuitem', { name: 'Edit metadata', exact: true }).click()
  const dialog = page.getByRole('dialog', { name: 'Edit metadata', exact: true })
...
test('Processes merges job types and preserves the old downloads route', async ({ page }) => {
  ...
  await page.getByRole('combobox', { name: 'Process type' }).selectOption('conversion')
  ...
  await page.getByRole('combobox', { name: 'Process type' }).selectOption('all')
...
async function mockExports(page: Page) {
  await page.route('**/api/files/*/processing', route => route.fulfill({ json: { revision: 'revision-1', duration_seconds: 12, video: true, audio: true, width: 1280, height: 720, formats: ['mp4', 'mkv', 'm4a', 'mp3', 'flac', 'wav'], copy_formats: ['mp4', 'mkv', 'm4a'], notices: ['container_metadata'] } }))
}

test('conversion menu keyboard, dirty guard, failure and accepted feedback', async ({ page }) => {
  await mockApi(page)
  await mockExports(page)
  let attempts = 0
  await page.route('**/api/files/*/processes', route => {
    attempts++
    if (attempts === 1) return route.fulfill({ status: 409, json: { error: { message: 'Changed source' } } })
    return route.fulfill({ status: 202, json: { id: 'export-1', kind: 'conversion', source_file_id: completedFixture[0].id, output_file_id: null, title: 'My audio', state: 'queued', stage: 'queued', progress_percent: null, eta_seconds: null, created_at: '2026-09-24T00:00:00Z', updated_at: '2026-09-24T00:00:00Z', started_at: null, finished_at: null, error: null } })
  })
  await page.goto('/completed')
  const actions = page.getByRole('button', { name: 'Actions for Fixture media' })
  await actions.focus()
  await page.keyboard.press('ArrowDown')
  await expect(page.getByRole('menuitem', { name: 'Edit metadata' })).toBeFocused()
  await page.keyboard.press('ArrowDown')
  await expect(page.getByRole('menuitem', { name: 'Convert', exact: true })).toBeFocused()
  await page.keyboard.press('Escape')
  await expect(actions).toBeFocused()
  await actions.click()
  await page.getByRole('menuitem', { name: 'Convert', exact: true }).click()
  const dialog = page.getByRole('dialog', { name: 'Convert', exact: true })
  await expect(page.locator('video')).toHaveCount(0)
  await expect(dialog.getByRole('button', { name: 'Start conversion' })).toBeDisabled()
  await dialog.getByLabel('Output type').selectOption('audio')
  await dialog.getByLabel('Format', { exact: true }).selectOption('mp3')
  await dialog.getByLabel('Output filename').fill('My audio')
  await page.keyboard.press('Escape')
  await expect(dialog.getByText('Discard your unsaved changes?')).toBeVisible()
  await dialog.getByRole('button', { name: 'Keep editing' }).click()
  await dialog.getByLabel('I understand these preservation limits.').check()
  await expect(dialog.getByRole('button', { name: 'Start conversion' })).toBeInViewport()
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true)
  await dialog.getByRole('button', { name: 'Start conversion' }).click()
  await expect(dialog.getByRole('alert')).toContainText('Could not start processing')
  await expect(dialog.getByLabel('Output filename')).toHaveValue('My audio')
  const request = page.waitForRequest(request => request.method() === 'POST' && request.url().endsWith('/processes'))
  await dialog.getByRole('button', { name: 'Start conversion' }).click()
  expect((await request).postDataJSON()).toMatchObject({ filename: 'My audio', format: 'mp3', stream_copy: false, acknowledge_omissions: true })
  await expect(dialog).toHaveCount(0)
  await expect(page).toHaveURL(/completed$/)
  await expect(page.getByRole('status').filter({ hasText: 'Added to Processes' })).toBeVisible()
  await page.getByRole('link', { name: 'View process' }).click()
  await expect(page.locator('#process-export-1')).toBeFocused()
  expect(attempts).toBe(2)
})

test('quick edit validates trim and submits combined rotate and mute with honest preview fallback', async ({ page }) => {
  await mockApi(page)
  await mockExports(page)
  await page.route('**/api/completed', route => route.fulfill({ json: [{ ...completedFixture[0], browser_playable: false, thumbnail_available: false }] }))
  await page.route('**/api/files/*/processes', route => route.fulfill({ status: 202, json: { id: 'edit-1', kind: 'edit', source_file_id: completedFixture[0].id, output_file_id: null, title: 'My edit', state: 'queued', stage: 'queued', progress_percent: null, eta_seconds: null, created_at: '2026-09-24T00:00:00Z', updated_at: '2026-09-24T00:00:00Z', started_at: null, finished_at: null, error: null } }))
  await page.goto('/completed')
  await page.getByRole('button', { name: 'Actions for Fixture media' }).click()
  await page.getByRole('menuitem', { name: 'Quick edit', exact: true }).click()
  const dialog = page.getByRole('dialog', { name: 'Quick edit', exact: true })
  await expect(dialog.getByRole('link', { name: 'Download', exact: true })).toHaveAttribute('href', `/api/files/${completedFixture[0].id}/download`)
  await expect(dialog.locator('video')).toHaveCount(0)
  await expect(dialog.getByLabel('Format', { exact: true })).toHaveValue('mp4')
  await dialog.locator('.editor-export > summary').click()
  await dialog.getByLabel('I understand these preservation limits.').check()
  await expect(dialog.getByRole('button', { name: 'Export edited copy' })).toBeDisabled()
  await dialog.getByRole('checkbox', { name: 'Trim', exact: true }).check()
  await dialog.getByLabel('Start (seconds)').fill('4')
  await dialog.getByLabel('End (seconds)').fill('2')
  await expect(dialog.getByRole('button', { name: 'Export edited copy' })).toBeDisabled()
  await dialog.getByLabel('End (seconds)').fill('8')
  await dialog.getByRole('tab', { name: 'Frame', exact: true }).click()
  await dialog.getByRole('button', { name: 'Rotate clockwise', exact: true }).click()
  await dialog.getByRole('tab', { name: 'Audio', exact: true }).click()
  await dialog.getByLabel('Remove audio').check()
  await expect(dialog.getByRole('button', { name: 'Export edited copy' })).toBeInViewport()
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true)
  const request = page.waitForRequest(request => request.method() === 'POST' && request.url().endsWith('/processes'))
  await dialog.getByRole('button', { name: 'Export edited copy' }).click()
  expect((await request).postDataJSON()).toMatchObject({ format: 'mp4', stream_copy: false, edits: { start_seconds: 4, end_seconds: 8, rotate: 90, mute: true } })
  await expect(dialog).toHaveCount(0)
  await expect(page).toHaveURL(/completed$/)
})

test('quick edit combines crop resize and volume and constrains dimensions', async ({ page }) => {
  await mockApi(page)
  await mockExports(page)
  await page.route('**/api/files/*/processes', route => route.fulfill({ status: 500, json: { error: { message: 'Test failure retains inputs' } } }))
  await page.goto('/completed')
  await page.getByRole('button', { name: 'Actions for Fixture media' }).click()
  await page.getByRole('menuitem', { name: 'Quick edit', exact: true }).click()
  const dialog = page.getByRole('dialog', { name: 'Quick edit', exact: true })
  await dialog.getByRole('tab', { name: 'Frame', exact: true }).click()
  await dialog.getByLabel('Crop', { exact: true }).check()
  await dialog.getByText('Exact crop values', { exact: true }).click()
  await dialog.getByLabel('Crop Left (px)', { exact: true }).fill('1000')
  await dialog.locator('.editor-export > summary').click()
  await dialog.getByLabel('I understand these preservation limits.').check()
  await expect(dialog.getByRole('button', { name: 'Export edited copy' })).toBeDisabled()
  await dialog.getByLabel('Crop Left (px)', { exact: true }).fill('0')
  await dialog.getByLabel('Crop Width (px)', { exact: true }).fill('960')
  await dialog.getByLabel('Crop Height (px)', { exact: true }).fill('540')
  await dialog.getByRole('tab', { name: 'Frame', exact: true }).click()
  await dialog.getByRole('button', { name: 'Rotate clockwise', exact: true }).click()
  await dialog.getByLabel('Resize', { exact: true }).check()
  await dialog.getByText('Custom dimensions', { exact: true }).click()
  await dialog.getByLabel('Resize Width (px)', { exact: true }).fill('270')
  await expect(dialog.getByLabel('Resize Height (px)', { exact: true })).toHaveValue('480')
  await expect(dialog.getByLabel('Resize Height (px)', { exact: true })).toHaveAttribute('readonly', '')
  await dialog.getByRole('tab', { name: 'Audio', exact: true }).click()
  await dialog.getByLabel('Adjust volume', { exact: true }).check()
  await dialog.getByLabel('Volume (%)').fill('50')
  const request = page.waitForRequest(request => request.method() === 'POST' && request.url().endsWith('/processes'))
  await dialog.getByRole('button', { name: 'Export edited copy' }).click()
  expect((await request).postDataJSON().edits).toEqual({ rotate: 90, mute: false, crop: { x: 0, y: 0, width: 960, height: 540 }, resize: { width: 270, height: 480 }, volume: 0.5 })
  await expect(dialog.getByRole('alert')).toContainText('Could not start processing')
  await dialog.getByRole('tab', { name: 'Frame', exact: true }).click()
  await dialog.getByText('Custom dimensions', { exact: true }).click()
  await expect(dialog.getByLabel('Resize Width (px)', { exact: true })).toHaveValue('270')
  await dialog.getByRole('tab', { name: 'Audio', exact: true }).click()
  await dialog.getByLabel('Remove audio').check()
  await expect(dialog.getByLabel('Volume (%)')).toHaveCount(0)
})

for (const [locale, convert, edit, submit, rotate, crop, resize, volume, acknowledge] of [
  ['ru', 'Конвертировать', 'Быстрая правка', 'Экспортировать копию', 'Поворот по часовой стрелке', 'Кадрировать', 'Изменить размер', 'Изменить громкость', 'Я понимаю ограничения сохранения данных.'],
  ['tg', 'Табдил додан', 'Таҳрири зуд', 'Содироти нусхаи таҳриршуда', 'Гардондан бо самти соат', 'Кадрбандӣ', 'Тағйири андоза', 'Тағйири баландии садо', 'Ман маҳдудиятҳои нигоҳдории маълумотро мефаҳмам.'],
]) {
  test(`export dialogs are localized, focus trapped and contained in ${locale}`, async ({ page }) => {
    await page.addInitScript(locale => localStorage.setItem('fetch.locale', locale), locale)
    await mockApi(page)
    await mockExports(page)
    await page.goto('/completed')
    const actions = page.locator('.metadata-pencil')
    await actions.click()
    await page.getByRole('menuitem', { name: convert, exact: true }).click()
    await expect(page.getByRole('dialog', { name: convert, exact: true })).toBeVisible()
    await page.keyboard.press('Escape')
    await expect(actions).toBeFocused()
    await actions.click()
    await page.getByRole('menuitem', { name: edit, exact: true }).click()
    const dialog = page.getByRole('dialog', { name: edit, exact: true })
    await dialog.locator('#editor-tab-frame').click()
    await dialog.getByRole('button', { name: rotate, exact: true }).click()
    await dialog.getByLabel(crop, { exact: true }).check()
    await dialog.getByLabel(resize, { exact: true }).check()
    await dialog.locator('#editor-tab-audio').click()
    await dialog.getByLabel(volume, { exact: true }).check()
    await dialog.locator('.editor-export > summary').click()
    await dialog.getByLabel(acknowledge).check()
    const button = dialog.getByRole('button', { name: submit, exact: true })
    await expect(button).toBeInViewport()
    await button.focus()
    await page.keyboard.press('Tab')
    expect(await dialog.evaluate(element => element.contains(document.activeElement))).toBe(true)
    await expect(dialog).not.toContainText('quickEdit.')
    await expect(dialog).not.toContainText('exports.')
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true)
    await page.screenshot({ path: test.info().outputPath(`quick-edit-${locale}.png`) })
  })
}

test('audio editing hides video controls and requires a supported output choice', async ({ page }) => {
  await mockApi(page)
  await page.route('**/api/completed', route => route.fulfill({ json: [{ ...completedFixture[0], filename: 'music.ogg', mime_type: 'audio/ogg', browser_playable: false }] }))
  await page.route('**/api/files/*/processing', route => route.fulfill({ json: { revision: 'a', duration_seconds: 12, video: false, audio: true, width: null, height: null, formats: ['m4a', 'mp3', 'flac', 'wav'], copy_formats: [], notices: ['container_metadata'] } }))
  await page.goto('/completed')
  await page.locator('.metadata-pencil').click()
  await page.getByRole('menuitem', { name: 'Quick edit', exact: true }).click()
  const dialog = page.getByRole('dialog', { name: 'Quick edit', exact: true })
  await expect(dialog.getByLabel('Rotate clockwise')).toHaveCount(0)
  await expect(dialog.getByLabel('Crop', { exact: true })).toHaveCount(0)
  await expect(dialog.getByLabel('Remove audio')).toHaveCount(0)
  await dialog.getByRole('tab', { name: 'Audio', exact: true }).click()
  await dialog.getByLabel('Adjust volume', { exact: true }).check()
  await dialog.locator('.editor-export > summary').click()
  await dialog.getByLabel('I understand these preservation limits.').check()
  await expect(dialog.getByRole('button', { name: 'Export edited copy' })).toBeDisabled()
  await dialog.getByLabel('Format', { exact: true }).selectOption('flac')
  await expect(dialog.getByRole('button', { name: 'Export edited copy' })).toBeEnabled()
})

test('conversion supports copy-only runtime and prevents duplicate submits', async ({ page }) => {
  await mockApi(page)
  await page.route('**/api/files/*/processing', route => route.fulfill({ json: { revision: 'a', duration_seconds: 12, video: true, audio: true, width: 1280, height: 720, formats: [], copy_formats: ['mp4', 'mkv'], notices: ['container_metadata'] } }))
  let requests = 0
  let release: (() => void) | undefined
  const pending = new Promise<void>(resolve => { release = resolve })
  await page.route('**/api/files/*/processes', async route => { requests++; expect(route.request().postDataJSON().stream_copy).toBe(true); await pending; await route.fulfill({ status: 500, json: { error: { code: 'OUTPUT_DIRECTORY_UNAVAILABLE' } } }) })
  await page.goto('/completed')
  await page.locator('.metadata-pencil').click()
  await page.getByRole('menuitem', { name: 'Convert', exact: true }).click()
  const dialog = page.getByRole('dialog', { name: 'Convert', exact: true })
  await expect(dialog.getByLabel('Copy compatible streams without re-encoding')).toBeChecked()
  await dialog.getByLabel('I understand these preservation limits.').check()
  await dialog.getByRole('button', { name: 'Start conversion' }).click()
  await expect(dialog.getByRole('button', { name: 'Start conversion' })).toBeDisabled()
  ...
  await expect(dialog).toBeVisible()
  release?.()
  await expect(dialog.getByRole('alert')).toContainText('Check free space')
  expect(requests).toBe(1)
})

test('interactive quick editor previews a selection, moves crop handles and resets edits', async ({ page }) => {
  await page.emulateMedia({ colorScheme: 'dark' })
  await mockApi(page)
  await mockExports(page)
  const source = await readFile('e2e/fixtures/editor-source.webm')
  await page.route('**/api/files/*/stream', route => {
    const range = /bytes=(\d+)-(\d*)/.exec(route.request().headers().range ?? '')
    const start = Number(range?.[1] ?? 0), end = range?.[2] ? Number(range[2]) : source.length - 1
    return route.fulfill({ status: range ? 206 : 200, body: source.subarray(start, end + 1), contentType: 'video/webm', headers: { 'Accept-Ranges': 'bytes', ...(range ? { 'Content-Range': `bytes ${start}-${end}/${source.length}` } : {}) } })
  })
  await page.goto('/completed')
  await page.locator('.metadata-pencil').click()
  await page.getByRole('menuitem', { name: 'Quick edit', exact: true }).click()
  const dialog = page.getByRole('dialog', { name: 'Quick edit', exact: true })
  await expect(dialog.getByRole('button', { name: 'Play source', exact: true })).toBeEnabled()
  await expect(dialog.getByRole('button', { name: 'Review export' })).toBeDisabled()
  const startHandle = dialog.getByRole('slider', { name: 'Trim start handle' })
  await startHandle.focus()
  await page.keyboard.press('ArrowRight')
  await expect(dialog.getByRole('checkbox', { name: 'Trim', exact: true })).toBeChecked()
  await dialog.getByLabel('Start (seconds)').fill('1')
  await dialog.getByLabel('End (seconds)').fill('1.5')
  await dialog.getByRole('button', { name: 'Play selection' }).click()
  await expect.poll(() => dialog.locator('video').evaluate(video => (video as HTMLVideoElement).currentTime)).toBeGreaterThanOrEqual(1.5)
  await expect.poll(() => dialog.locator('video').evaluate(video => (video as HTMLVideoElement).paused)).toBe(true)
  await dialog.getByRole('tab', { name: 'Frame', exact: true }).click()
  await dialog.getByRole('group', { name: 'Crop aspect ratio' }).getByRole('button', { name: '1:1', exact: true }).click()
  const crop = dialog.getByRole('button', { name: 'Move crop area', exact: true })
  await crop.focus()
  await page.keyboard.press('ArrowRight')
  await dialog.getByText('Exact crop values', { exact: true }).click()
  await expect(dialog.getByLabel('Crop Left (px)', { exact: true })).toHaveValue('282')
  ...
  await page.mouse.up()
  expect(Number(await dialog.getByLabel('Crop Left (px)', { exact: true }).inputValue())).toBeGreaterThan(282)
  const handle = dialog.getByRole('button', { name: 'Resize crop area', exact: true })
  ...
  await page.mouse.up()
  expect(Number(await dialog.getByLabel('Crop Width (px)', { exact: true }).inputValue())).toBeLessThan(720)
  await dialog.getByRole('group', { name: 'Crop aspect ratio' }).getByRole('button', { name: '1:1', exact: true }).click()
  await dialog.getByRole('group', { name: 'Resize scale' }).getByRole('button', { name: '50%', exact: true }).click()
  await expect(dialog.locator('.editor-summary')).toContainText('360 × 360')
  await dialog.getByRole('button', { name: 'Rotate clockwise', exact: true }).click()
  await expect(dialog.locator('.editor-summary')).toContainText('90°')
  await page.screenshot({ path: test.info().outputPath('interactive-editor.png') })
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true)
  await dialog.getByRole('button', { name: 'Review export' }).click()
  await expect(dialog.getByLabel('Output filename')).toBeFocused()
  await expect(dialog.getByRole('button', { name: 'Export edited copy' })).toBeDisabled()
  await dialog.getByLabel('I understand these preservation limits.').check()
  await expect(dialog.getByRole('button', { name: 'Export edited copy' })).toBeEnabled()
  await dialog.getByRole('button', { name: 'Reset edits' }).click()
  await expect(dialog.getByRole('button', { name: 'Export edited copy' })).toBeDisabled()
})

test('History includes persisted downloads and all finished processing kinds after reload', async ({ page }) => {
  await mockApi(page)
  const base = { source_file_id: completedFixture[0].id, output_file_id: null, progress_percent: null, eta_seconds: null, created_at: '2026-09-24T00:00:00Z', started_at: null, finished_at: null, error: null }
  const jobs = [
    { ...base, id: 'edit', kind: 'edit', title: 'Edited copy', state: 'completed', stage: 'completed', updated_at: '2026-09-24T05:00:00Z' },
    { ...base, id: 'metadata', kind: 'metadata', title: 'Saved tags', state: 'completed', stage: 'completed', updated_at: '2026-09-24T04:00:00Z' },
    { ...base, id: 'conversion', kind: 'conversion', title: 'Converted copy', state: 'completed', stage: 'completed', updated_at: '2026-09-24T03:00:00Z' },
    ...['failed', 'cancelled', 'interrupted', 'running', 'queued'].map(state => ({ ...base, id: state, kind: 'edit', title: `${state} export`, state, stage: state, updated_at: '2026-09-24T02:00:00Z' })),
  ]
  await page.route('**/api/processes', route => route.fulfill({ json: jobs }))
  await page.route('**/api/history', route => route.fulfill({ json: [{ id: 'download', title: 'Original download', url: 'https://example.test/media', mode: 'video', status: 'completed', updated_at: '2026-09-24T01:00:00Z' }] }))
  await page.goto('/history')
  for (let visit = 0; visit < 2; visit++) {
    const rows = page.locator('tbody tr')
    await expect(rows).toHaveCount(7)
    await expect(rows.nth(0)).toContainText('Edited copy')
    await expect(rows.nth(0)).toContainText('Edits')
    await expect(rows.nth(1)).toContainText('Metadata updates')
    await expect(rows.nth(2)).toContainText('Conversions')
    await expect(rows.last()).toContainText('Original download')
    await expect(page.locator('tbody')).not.toContainText('running export')
    await expect(page.locator('tbody')).not.toContainText('queued export')
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true)
    const filter = page.getByRole('combobox', { name: 'Process type' })
    for (const [kind, title, count] of [['download', 'Original download', 1], ['conversion', 'Converted copy', 1], ['edit', 'Edited copy', 4], ['metadata', 'Saved tags', 1]] as const) {
