...
<template>
  <div class="modal" role="dialog" aria-modal="true" aria-labelledby="network-qr-title">
    <button class="modal-backdrop" type="button" :aria-label="t('qr.close')" @click="emit('close')"></button>
    <div ref="panel" class="modal-panel max-w-md" tabindex="-1">
      <div class="flex items-start justify-between gap-4">
        <div class="flex items-center gap-3"><div class="empty-icon"><QrCode :size="20" /></div><div><h2 id="network-qr-title" class="text-base font-semibold">{{ t('qr.title') }}</h2><p class="mt-1 text-xs text-muted">{{ t('qr.description') }}</p></div></div>
        <button class="icon-btn shrink-0" type="button" :aria-label="t('qr.close')" @click="emit('close')"><X :size="16" /></button>
      </div>

      <div v-if="settings.loading" class="mt-6 flex min-h-64 items-center justify-center text-xs text-muted">{{ t('qr.finding') }}</div>
      <div v-else-if="selected" class="mt-6">
        ...
        <div class="mt-5 flex gap-2">
          ...
          <button class="secondary-btn shrink-0" type="button" @click="copyUrl"><Check v-if="copied" :size="15" /><Copy v-else :size="15" />{{ copied ? t('qr.copied') : t('qr.copy') }}</button>
        </div>
        ...
        <div class="warning-panel mt-5"><TriangleAlert class="shrink-0 text-amber-500" :size="18" /><p class="text-[11px] leading-5">{{ t('qr.warning') }}</p></div>
      </div>
      <div v-else class="mt-6 rounded-xl border border-border bg-panel-2 p-6 text-center">
        ...
        <h3 class="mt-4 text-sm font-semibold">{{ t('qr.unavailable') }}</h3>
        <p class="mt-2 text-xs leading-5 text-muted">{{ t('qr.unavailableHelp') }}</p>
        <RouterLink class="secondary-btn mt-4" to="/settings#network" @click="emit('close')">{{ t('qr.openNetwork') }}</RouterLink>
      </div>
