...
<template>
  <div v-if="runtime.needsSetup || busy" class="modal" role="dialog" aria-modal="true" aria-labelledby="runtime-setup-title">
    ...
    <div class="modal-panel max-w-lg">
      ...
      <h2 id="runtime-setup-title" class="mt-5 text-xl font-semibold">Set up Fetch runtime</h2>
      <p class="mt-2 text-sm leading-6 text-muted">Fetch needs a real yt-dlp executable. The official standalone artifact is checksum-verified before it replaces any working version.</p>
      <div class="runtime-card mt-6">
        <div class="flex items-center gap-3">
          <div class="empty-icon"><Download :size="19" /></div>
          <div class="min-w-0 flex-1"><div class="text-sm font-semibold">yt-dlp</div><div class="mt-1 text-xs text-muted">{{ current?.version ?? current?.status ?? 'Checking…' }}</div></div>
      ...
      <button class="primary-btn mt-5 w-full py-3" type="button" :disabled="busy" @click="runtime.act('yt-dlp', current?.status === 'failed' ? 'repair' : 'install')">
        <RefreshCw v-if="busy" class="animate-spin" :size="16" />{{ busy ? 'Installing…' : current?.status === 'failed' ? 'Repair yt-dlp' : 'Install yt-dlp' }}
      </button>
    </div>
  </div>
</template>
