    ...
    <div class="player-dialog" role="dialog" aria-modal="true" :aria-labelledby="`player-title-${file.id}`">
      ...
      <section ref="panel" class="player-panel">
        ...
        <div class="player-stage">
          <video v-if="!mediaFailed && file.mime_type.startsWith('video/')" class="player-video" controls autoplay playsinline preload="metadata" :poster="file.thumbnail_available && !thumbnailFailed ? `/api/files/${file.id}/thumbnail` : undefined" :src="`/api/files/${file.id}/stream`" @error="mediaFailed = true"></video>
          <div v-else class="player-audio-stage">
            ...
            <audio v-if="!mediaFailed && file.mime_type.startsWith('audio/')" class="w-full" controls autoplay preload="metadata" :src="`/api/files/${file.id}/stream`" @error="mediaFailed = true"></audio>
        ...
        <footer class="player-footer">
          <div class="min-w-0 flex-1"><div class="truncate text-xs font-medium">{{ file.filename }}</div><div class="mt-1 text-[11px] text-muted">{{ file.mime_type }} · {{ bytes(file.size_bytes) }}</div></div>
