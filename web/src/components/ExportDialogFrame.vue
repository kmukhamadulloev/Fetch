...
let overflow = ''
function close() { if (!props.busy) { if (props.dirty) discard.value = true; else emit('close') } }
...
function keydown(event: KeyboardEvent) {
  ...
  const items = [...(panel.value?.querySelectorAll<HTMLElement>('button:not(:disabled), input:not(:disabled), select:not(:disabled), a[href], [tabindex="0"]') ?? [])].filter(item => item.getClientRects().length)
...
<template>
  <Teleport to="body">
    <div class="modal metadata-modal" role="dialog" aria-modal="true" aria-labelledby="export-title" @keydown="keydown">
      ...
      <form ref="panel" class="modal-panel metadata-panel" :style="wide ? { maxWidth: '60rem' } : undefined" tabindex="-1" :aria-busy="busy" @submit.prevent="!disabled && !busy && emit('submit')">
        ...
        <div class="min-h-0 space-y-4 overflow-y-auto p-5"><slot /><div v-if="discard" class="error-panel" role="alert"><p>{{ t('metadata.discardPrompt') }}</p><div class="mt-3 flex gap-2"><button type="button" class="secondary-btn" @click="discard = false">{{ t('metadata.keepEditing') }}</button><button type="button" class="danger-btn" @click="emit('close')">{{ t('metadata.discard') }}</button></div></div></div>
