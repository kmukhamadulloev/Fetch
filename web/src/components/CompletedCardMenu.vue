...
const emit = defineEmits<{ metadata: []; convert: []; quickEdit: [] }>()
const { t } = useI18n()
...
<template>
  <div ref="root" @click.stop @keydown="keydown" @focusout="(event) => { if (!root?.contains(event.relatedTarget as Node)) close() }">
    ...
    <div v-if="open" :id="`file-menu-${id}`" role="menu" class="absolute left-2.5 top-14 z-20 grid min-w-44 gap-1 rounded-xl border border-border bg-panel p-1.5 shadow-xl">
      <button role="menuitem" tabindex="-1" class="rounded-lg px-3 py-2 text-left text-sm hover:bg-panel-2" @click="close(true); emit('metadata')">{{ t('metadata.edit') }}</button>
      <button role="menuitem" tabindex="-1" class="rounded-lg px-3 py-2 text-left text-sm hover:bg-panel-2" @click="close(true); emit('convert')">{{ t('exports.convert') }}</button>
      <button role="menuitem" tabindex="-1" class="rounded-lg px-3 py-2 text-left text-sm hover:bg-panel-2" @click="close(true); emit('quickEdit')">{{ t('quickEdit.title') }}</button>
    </div>
  </div>
</template>
