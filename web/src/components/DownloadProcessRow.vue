...
const props = defineProps<{ job: DownloadJob; now: number }>()
const library = useLibraryStore()
const output = computed(() => library.completed.find(file => file.job_id === props.job.id))
function elapsed() { return duration(((['completed', 'failed', 'stopped'].includes(props.job.status) ? Date.parse(props.job.updated_at) : props.now) - Date.parse(props.job.created_at)) / 1000) }
    ...
    <div class="flex items-start gap-4">
      ...
      <div class="min-w-0 flex-1">
        ...
        <div class="mt-3 flex flex-wrap gap-x-4 gap-y-2 text-[11px] text-muted">
          <span class="helper">{{ t('processing.types.download') }}</span>
        ...
        <p class="mt-2 text-[11px] text-muted">{{ t('processing.elapsed', { duration: elapsed() }) }}</p>
        <RouterLink v-if="output" class="secondary-btn mt-3" :to="{ path: '/completed', query: { file: output.id } }">{{ t('processing.viewFile') }}</RouterLink>
