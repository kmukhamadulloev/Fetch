...
const quickEditing = ref<CompletedFile | null>(null)
  ...
  <section class="min-w-0">
    <template v-if="requestedPlaylistId && activePlaylist">
      ...
      <div class="mt-6 grid min-w-0 grid-cols-[minmax(0,1fr)] gap-4 sm:grid-cols-2 xl:grid-cols-3" data-playlist-gallery>
        <CompletedMediaCard v-for="file in activePlaylist.files" :key="file.id" :file="file" @edit="editing = $event" @convert="converting = $event" @quick-edit="quickEditing = $event" @play="selected = $event" @delete="pendingDelete = $event" />
      </div>
    </template>
    ...
    <template v-else-if="!requestedPlaylistId">
      ...
      <div v-else-if="libraryEntries.length" class="completed-grid mt-6 grid min-w-0 grid-cols-[minmax(0,1fr)] gap-4 sm:grid-cols-2 xl:grid-cols-3">
        <template v-for="entry in libraryEntries" :key="entry.kind === 'file' ? entry.file.id : `playlist-${entry.playlist.id}`">
          <CompletedMediaCard v-if="entry.kind === 'file'" :file="entry.file" @edit="editing = $event" @convert="converting = $event" @quick-edit="quickEditing = $event" @play="selected = $event" @delete="pendingDelete = $event" />
    ...
    <QuickEditDialog v-if="quickEditing" :file="quickEditing" @close="quickEditing = null" @accepted="acceptedProcess = $event.id" />
